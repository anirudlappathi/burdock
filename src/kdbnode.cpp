...
KDBNode::KDBNode(std::string folderlocation, u_int dim, size_t nodeId): 
_file_location("dbfiles" + __id_to_n_len_id(S_IDSIZE, nodeId) + ".brdk"),
...
KDBNode::KDBNode(std::string folderlocation, bool is_written, size_t nodeId, u_int dim, u_char data_type_byte_count):
    _file_location("dbfiles" + __id_to_n_len_id(S_IDSIZE, nodeId) + ".brdk"),
    _folder_location(folderlocation),
    _loaded(is_written),
    _node_id(nodeId),
...
size_t KDBNode::__get_node_id() {
    return _node_id;
}

void KDBNode::filelocation(std::string filelocation) {
...
u_char KDBNode::datatypebytecount() {
