...
VectorDatabase::VectorDatabase(const std::string &folderPath) : _database_name(folderPath),
                                                                _kdbtree(folderPath)
{
...
VectorDatabase::VectorDatabase(const std::string &folderPath, size_t dim) : _database_name(folderPath),
                                                                            _id(folderPath),
                                                                            _folder_path(folderPath),
...
VectorDatabase::VectorDatabase(const std::string &database_name, const std::string &folder_path, size_t file_count, KDBTree &&kdbtree) : _database_name(database_name),
                                                                                                                                         _id("1"),
                                                                                                                                         _folder_path(folder_path),
                                                                                                                                         _kdbtree(std::move(kdbtree)),
                                                                                                                                         _file_count(file_count) {}

VectorDatabase::VectorDatabase(VectorDatabase &&other) noexcept : _database_name(other._database_name),
                                                                  _id(other._id),
...
void VectorDatabase::__delete_folder(bool coutWarning) const
{
    __delete_folder(_folder_path, coutWarning);
}

void VectorDatabase::__delete_folder(const std::string &folder_name, bool coutWarning) const
{
    if (coutWarning && (!std::__fs::filesystem::exists(_folder_path) || !std::__fs::filesystem::is_directory(_folder_path)))
    {
        std::cout << "WARNING [vectordb.cpp][__delete_folder]: Folder name \"" + _folder_path + "\" does not exist to delete" << std::endl;
    }
    std::__fs::filesystem::remove_all(folder_name);
}

int VectorDatabase::__get_node_id_from_filepath(std::__fs::filesystem::path file_path) const
{
...
KDBNode *VectorDatabase::__read_files_left_to_right(std::vector<std::__fs::filesystem::path> &files, int &fileIndex, int &nodeCount, int &dimensions, int layer, size_t &vectorCount)
{
    assert(layer > 0);
    if (layer <= 1)
    {
        ...
        currNode->set_data_type_byte_count(datatypeByteCount);
        nodeCount++;
...
u_int VectorDatabase::dimensions() const
{
    return _kdbtree.dimensions();
}

size_t VectorDatabase::layers() const
{
    return _kdbtree.layers();
}

size_t VectorDatabase::vectors() const
{
...
std::vector<std::__fs::filesystem::path> VectorDatabase::__get_folder_files(const std::string &folderpath) const
{
    return _kdbtree.__get_folder_files(folderpath);
}

VectorDatabase &VectorDatabase::__load_all(const std::string &folderpath)
{
    std::vector<std::__fs::filesystem::path> files = __get_folder_files(folderpath);
    int layers = log2(files.size()) + 1;
    int dimensions = -1;
    ...
    VectorDatabase *vdb = new VectorDatabase(folderpath, folderpath, files.size(), std::move(*kdbtree));
    return *vdb;
}