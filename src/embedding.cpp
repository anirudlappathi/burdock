...
Embedding::Embedding(u_int dim) : _magnitude(0),
                                  dimensions(dim)
{
  // PURELY TESTING
...
Embedding::Embedding(Embedding &other) : _magnitude(0)
{
  if (this == &other)
  {
    return;
...
Embedding::Embedding(std::vector<float> &insert_embedding) : _magnitude(0),
                                                             dimensions(insert_embedding.size())
{
  ...
  for (u_int i = 0; i < dimensions; i++)
  {
    ...
    _magnitude += _embedding[i] * _embedding[i];
  }
  _magnitude = sqrt(_magnitude);
}

Embedding::Embedding(std::vector<float> &insert_embedding, const std::string &metadata) : _metadata(metadata),
                                                                                          _magnitude(0),
                                                                                          dimensions(insert_embedding.size())
{
...
Embedding::Embedding(float *insert_embedding, u_int dim, const std::string &metadata) : _embedding(insert_embedding),
                                                                                        _metadata(metadata),
                                                                                        _magnitude(0),
                                                                                        dimensions(dim) {}
...
float *Embedding::get_embedding() const
{
  return _embedding;
}

std::string Embedding::get_metadata() const
{
  return _metadata;
}
...
float Embedding::cosine_similarity(Embedding *other) const
{
  if (_magnitude == true && other->_magnitude == true)
  {
    return __dot_product(other) / (_magnitude * other->_magnitude);
  }
  if (_magnitude == false ^ other->_magnitude == false)
  {
