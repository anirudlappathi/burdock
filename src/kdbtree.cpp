...
KDBTree::KDBTree(std::string folderpath):
    _root(nullptr),
    _vector_count(0),
    _layers_count(1),
    _dim(0),
...
KDBTree::KDBTree(u_int dim, std::string folderpath):
    _root(nullptr),
    _vector_count(0),
    _layers_count(1),
    _dim(dim),
...
KDBTree::KDBTree(u_int dim, std::string folderpath, u_int starting_layers):
    _root(nullptr),
    _vector_count(0),
...
KDBTree::KDBTree(KDBNode* root, size_t vectorCount, int layers, std::string folderpath, int nodeCount):
    _root(root),
...
int KDBTree::__get_node_id_from_filepath(std::filesystem::path file_path) {
...
std::vector<std::filesystem::path> KDBTree::__get_folder_files(std::string folderpath) {
...
KDBNode* KDBTree::__load_node(std::string folderpath, int fileIndex) {
...
void KDBTree::folderpath(std::string folderpath) {
    _folderpath = folderpath;
}
...
bool KDBTree::insertEmbedding(Embedding& embedding) {
    ...
    _vector_count++;
    if (DEBUG) { std::cout << _layers_count << pow(2, _dim - 1) << std::endl; std::cout << (_layers_count < pow(2, _dim - 1)) << std::endl; }
    if (curr->size() >= _max_loaded_embeddings && _layers_count < pow(2, _dim - 1)) {    
        if (DEBUG) { std::cout << "NOTE [kdbtree.cpp][insertEmbedding]: Embedding limit reached (" + std::to_string(curr->size()) + ", " + std::to_string(curr->__get_node_id()) + ")" << std::endl; }
...
KDBNode* KDBTree::__read_files_left_to_right(std::vector<std::filesystem::path>& files, int& fileIndex, int& nodeCount, int& dimensions, int layer, size_t& vectorCount) {
    assert(layer > 0);
    if (layer == 1) {
        // make kdbnode reading file
        std::filesystem::path filePath = files.at(fileIndex);
        ...
        int datatypeByteCount = int(datatypeByteCountRead);
...
std::pair<Embedding*, float> KDBTree::query(std::string folderpath, Embedding& queryEmbedding) {
    int fileIndex = 0;
    KDBNode* curr = _root;
    int dim = 0;
    while (!curr->isleaf()) {
    ...
    if (!curr->loaded() && std::filesystem::exists(folderpath)) {
        curr->clear();
...
std::ostream& operator<<(std::ostream& os, KDBTree& obj) {
    const size_t layer_count = obj.layers();
